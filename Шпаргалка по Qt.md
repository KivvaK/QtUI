...
Там же проинициализируем "инструменты" рисования:
```cpp
...
pmi->setPixmap(image);
```
При параметрах инструментов, укаханных в п. 3, изображение в gvDrawer закрасится серым цветом, и на нём появится жёлтый треугольник.
